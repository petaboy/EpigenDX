...
def highlight(sequence,summary,exon_utr,snp):
    import re

    #Sequence prep
    sequence = sequence[17:-1]
    ...
    if int(summary[0]['Strand']) == -1:
        ...
        for row in snp:
            ...
            for term in range(start_pos,end_pos+end_term_offset):
                start_snp_link_updated = start_snp_link %(row['Variant Name'][3:])
...
if __name__ == "__main__":
    ...
    #text = highlight(sequence,summary,exon_utr,snp)
    f.write(highlight(sequence,summary,exon_utr,snp))
    f.close()
    #print text